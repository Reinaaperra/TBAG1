    ...
    def handle_player_actions(self):
        ...
        while True:
            ...
            if command in ["north", "south", "east", "west", "up", "down"]:
                ...
                for index, chosen_weapon in enumerate(player_inventory["weapons"]):
                    print(f"{index + 1}. {chosen_weapon}")

                try:
                    weapon_choice = int(input("> ")) - 1
                    selected_weapon = player_inventory["weapons"][weapon_choice].lower()
                    ...
                    if inhabitant.fight(selected_weapon):
                        ...
                        item_names = ', '.join(item._name for item in acquired_items)  
